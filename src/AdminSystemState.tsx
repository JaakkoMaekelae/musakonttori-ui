...
const TONE: Record<SystemStateKind, string> = {
 ...
 */
export function AdminSystemState({
  kind,
  title,
  description,
  action,
  ...
  return (
    <div
      ...
      className={cn(
        ...
        size === "zone" ? "h-full min-h-48 px-6 py-10" : "px-4 py-8",
        className,
      )}
    >
      {isLoading ? (
        <span
          aria-hidden="true"
          className="mk-state-track relative h-[3px] w-32 overflow-hidden rounded-full bg-[var(--mk-palette-border-subtle,rgba(255,255,255,0.08))]"
        >
          <style>{`
        ...
        <span
          ...
          style={{ background: TONE[kind] }}
        />
      )}

      <p
        className="text-[0.8125rem] font-semibold text-[var(--mk-palette-text-primary,#F0F0F3)]"
        style={kind === "error" ? { color: TONE.error } : undefined}
      >
        {title}
      </p>

      {description && (
        <p className="max-w-xs text-xs text-[var(--mk-palette-text-secondary,#B0B3C1)]">
          {description}
        </p>
      )}
